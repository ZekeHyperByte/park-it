...
E-Parking v2 is a parking management system for Indonesian parking facilities. It supports e-money cards (Mandiri eMoney, BRI Brizzi, BNI TapCash, BCA Flazz), cash, and RFID member cards. The system controls physical parking gates, cameras, thermal printers, barcode scanners, and LED displays.
...
Single `Gate` table with `direction='IN'|'OUT'` — replaces old `GateIn`/`GateOut` split. Old tables were dropped in migration `f0bd4bac1599`. Query by direction: `GET /api/gates?direction=IN`.
...
- **Python:** ruff for linting (line-length 100, py312 target), mypy strict mode, Google-style docstrings
- **Database:** async SQLAlchemy with asyncpg, `DateTime(timezone=True)` always, JSONB for flexible configs, models inherit `Base + IntPKMixin + TimestampMixin`
...
RTK (Rust Token Killer) is installed for shell command output compression. It reduces token usage by 60-90% on common dev commands.
